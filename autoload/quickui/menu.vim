...
function! quickui#menu#create(opts)
	...
	let w = s:cmenu.width
	let opts = {"minwidth":w, "maxwidth":w, "minheight":1, "maxheight":1,"mode":"normal"}
	let opts.line = 1
	let opts.col = 1
...
" open menu
"----------------------------------------------------------------------
let g:quickui_mode = 'normal'
function! quickui#menu#open(...)
  :BeaconOff
	let opts = {}
	if a:0 > 0
		let opts.name = a:1
		let g:quickui_mode = a:2
	endif
	if g:quickui#core#has_nvim == 0
...
function! s:neovim_dropdown()
	...
	let item = s:cmenu.inst.items[s:cmenu.index]
	let opts = {'col': item.x + 1, 'line': 2, 'horizon':1, 'zindex':31100,'mode': g:quickui_mode}
	let opts.reserve = 1
	let opts.lazyredraw = 1
	let cfg = s:cmenu.current[item.name]
	let s:cmenu.dropdown = []
