...
" create menu object
"----------------------------------------------------------------------
function! s:filter(textlist,mode)
  let retrArrNormal = []
  let retrArrVisual = []
  for row in a:textlist
    let title = row[0]
    let action = row[1]
    if match(title,"[V]") != -1
      call add(retrArrVisual,[title,action])
    endif
    if match(title,"[N]") != -1
      call add(retrArrNormal,[title,action])
    endif
  endfor
  if a:mode == "normal"
    return retrArrNormal
  else
    return retrArrVisual
  endif
endfunction

function! quickui#context#open(textlist, opts) range
	let textlist = a:textlist
	let mode = a:opts["mode"]
	let a = s:filter(textlist,mode)
	if g:quickui#core#has_nvim == 0
		return s:vim_create_context(textlist, a:opts)
	else
		return s:nvim_create_context(a, a:opts)
	endif
endfunc
